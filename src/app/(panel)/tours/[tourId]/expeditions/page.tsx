...
import ExpeditionsTable from '@/app/(panel)/tourExpeditions/ExpeditionsTable'
...
const PageExpeditions: FC<PageExpeditionsProps> = ({ params }) => {
  ...
  const dd = params
  return (<>
    {item && <WrapDashBorder className='p-6 space-y-6' >
      <FormCard id="tour-id-expeditions" cardType={FormCardType.STATIC}
        title={<>
          <Link href={`/tours`} className='me-3 text-blue-700'><i className="fa-solid fa-left-long "></i></Link>
          {item.title}
        </>}
        bodyClassName='flex flex-col space-y-3'
      >

        <h2 className='text-2xl w-full'>{item.title}</h2>
        <div className='w-full text-lg'>{item.places}</div>
        <div className='w-full text-lg'>{item.duration} Days</div>
      ...
      <FormCard id="tours-expeditions-addnew" cardType={FormCardType.STATIC}
        title={t('Add/Edit Expedition')}
        bodyClassName='grid grid-cols-5 gap-4'>
        <div>bir</div><div>iki</div><div>uc</div><div>dort</div><div>bes</div>
      </FormCard>
      <ExpeditionsTable tourId={params.tourId} />
    </WrapDashBorder>
    }
  </>)
}
